...
def countdown(number_of_counts = 3, count_duration = 1.0):
...
def get_prediction(model, actions: list):
...
def play(actions: list, lost_against: dict):
    ...
    while game_continues:
        ...
        # read player action
        player_action =  get_prediction(model, actions)

        if player_action != 'Nothing':

            computer_action = get_computer_choice(actions=actions[:-1])
            won = get_winner(player_action=player_action, computer_action=computer_action, lost_against=lost_against)
